...
def control_checkpoint():
    """
    Check for control flags (stop/pause) and handle them appropriately.
    This function should be called periodically during long-running operations.
    """
    # STOP requested
    if stop_requested.is_set():
        log("[STOP] Requested during operation")
...
def copy_with_dedup(
    ...
    try:
        ...
        for h, dst in db.execute(
            "SELECT hash, dst_path FROM files WHERE hash IS NOT NULL"
        ):
        ...
        for src_path in safe_walk(src_root, db, ignore_patterns, retries=2):
            ...
            if record:
                ...
                if status == "pending":
                    log(
                        f"[RETRY-PREV] Incomplete previous attempt: {src_path}",
                        error=True,
                    )
        ...
        log(f"[ERROR] Unexpected exception in copy_with_dedup: {e}", error=True)
